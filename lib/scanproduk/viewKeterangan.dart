  ...
  void fetchData() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://192.168.8.121/ProjectScanner/lib/API/update_keterangan.php?no=${widget.selectedProject['id_lot']}'),
      );
  ...
  void _updateDataAndNavigateToListProject() async {
    try {
      final response = await http.post(
        Uri.parse(
            'http://192.168.8.121/ProjectScanner/lib/API/update_keterangan.php'),
        body: {
